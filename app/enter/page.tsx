...
export default function LiveEntry() {
  ...
  const updateRow = (rowIndex: number, columnId: string, rawValue: any) => {
    ...
    if (['down', 'dist', 'gnls'].includes(columnId)) {
    ...
    if (columnId === 'yardLine' && rowIndex > 0) {
      const prevYardLine = newData[rowIndex - 1].yardLine;
      newData[rowIndex - 1].gnls = calculateGainLoss(prevYardLine, formatted as NumericField);
        ...
        <div className="bg-zinc-900 border border-zinc-700 rounded-3xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-0">
              <thead className="bg-zinc-950 text-xs uppercase text-zinc-500 tracking-[0.25em]">
                {table.getHeaderGroups().map((headerGroup) => (
                  <tr key={headerGroup.id}>
                    {headerGroup.headers.map((header) => (
                      <th key={header.id} className="border-b border-zinc-800 px-4 py-3 text-left">
              ...
              <tbody>
                {table.getRowModel().rows.map((row, rowIndex) => (
                  <tr key={row.id} className="border-b border-zinc-800 hover:bg-zinc-900/80">
