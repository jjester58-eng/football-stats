...
import { ArrowLeft, TrendingUp, AlertTriangle, CheckCircle, Loader2, ChevronDown } from 'lucide-react';
...
function CompareBar({ label, scoutedPct, currentPct, count }: { label: string; scoutedPct: number; currentPct?: number; count: number }) {
  const delta = currentPct !== undefined ? currentPct - scoutedPct : undefined;
  const holding = delta !== undefined ? Math.abs(delta) <= 15 : undefined;
  return (
    <div className="grid grid-cols-[1fr_120px_120px] gap-4 items-center py-2 border-b border-zinc-800 last:border-0">
      <div className="text-sm text-zinc-200">{label} <span className="text-zinc-500 text-xs">({count} plays)</span></div>
      <div><Bar pct={scoutedPct} color="blue" /></div>
      <div>
        {currentPct !== undefined ? (
          <div className="flex items-center gap-2">
            <Bar pct={currentPct} color={holding ? 'emerald' : 'amber'} />
            {delta !== undefined && (
              <span className={`text-xs shrink-0 ${holding ? 'text-emerald-400' : 'text-amber-400'}`}>
                {delta > 0 ? '+' : ''}{delta}%
              </span>
            )}
          </div>
        ) : <span className="text-zinc-600 text-xs">No live data</span>}
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl overflow-hidden">
        <div className="px-5 py-4 border-b border-zinc-800 flex items-center justify-between">
          ...
          {viewMode === 'compare' && <div className="flex gap-4 text-xs"><span className="text-purple-400">■ Scouted</span><span className="text-emerald-400">■ Current</span></div>}
        </div>
